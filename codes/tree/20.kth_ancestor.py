...
class Solution:
    def kth_ancestor(self, root, node1, node2):
        ...
        if root.val == node1 or root.val == node2:
            return root
        left = self.kth_ancestor(root.left, node1, node2)
        right = self.kth_ancestor(root.right, node1, node2)
        if left and right:
            return root
        if left:
            return left
